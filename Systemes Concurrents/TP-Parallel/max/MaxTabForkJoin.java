    ...
    static class PartialMax extends RecursiveTask<Integer> {
        ...
        public Integer compute() {
            if ((end - start) > threshold) { // la tâche est trop grosse, on la décompose en deux
                PartialMax sp1 = new PartialMax(array, start, start + (end - start)/2, threshold);
                PartialMax sp2 = new PartialMax(array, start + (end - start)/2, end, threshold);
                sp1.fork();
                sp2.fork();
                // en commençant par sp2.join, la jvm va réutiliser ce thread pour exécuter sp2.
                int result = Math.max(sp2.join(), sp1.join());
                return result;
            } else {
