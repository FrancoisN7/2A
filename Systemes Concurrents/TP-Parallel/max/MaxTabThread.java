...
public class MaxTabThread {
    ...
    private static int computeMax(int[] array, int numberOfThreads) throws InterruptedException {
        ...
        // Le traitement du tableau est décomposé en segments de taille taskSize : [0..taskSize[, [taskSize..2*taskSize[ etc
        ...
        int start = 0;
        int end = Math.min(array.length, taskSize);
        while (start != array.length) {
            PartialMax partialMax = new PartialMax(array, start, end, results);
            start = end;
            end = end + Math.min(array.length-end, taskSize);
