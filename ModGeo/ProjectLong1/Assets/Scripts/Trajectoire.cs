...
public class Trajectoire : MonoBehaviour {
  ...
  private List<float> XSubdivision = new List<float>();
  private List<float> YSubdivision = new List<float>();
  private List<float> ZSubdivision = new List<float>();
  private int compteur;
  private GameObject[] ListePointsCrees;
  private int nbPointsParSub;
  private List<float> T = new List<float>();
  private List<float> TempsGlob = new List<float>();
  int indice = 0;
  ...
  void Start () {
    ...
    compteur = 0;
    ListePointsCrees = GameObject.FindGameObjectsWithTag("Points");
    ...
    nbPointsParSub = XSubdivision.Count/(ListePointsCrees.Length);

    // Création d'une liste T régulière
    for (int i=0 ; i<nbPointsParSub ; i++){
        T.Add(i*1/(nbPointsParSub-1));
    }

    // On concatène les lites T régulières pour tous les points créés
    for (int i=0 ; i<ListePointsCrees.Length ; i++){
        TempsGlob.AddRange(T);
    }
  }
    ...
    {
        //Debug.Log("Update");
        if (indice <= XSubdivision.Count && compteur<ListePointsCrees.Length){
            Debug.Log("Indice");
            Debug.Log(indice);
            Debug.Log("XSubdivision");
            Debug.Log(XSubdivision.Count);
            Debug.Log("ListPointsCrees");
            Debug.Log(ListePointsCrees.Length);
            Vector3 pos = new Vector3(XSubdivision[indice], ZSubdivision[indice], 
                                        YSubdivision[indice]);
            
            MainCamera.transform.position = pos;
            // Le problème est sur ListePointsCrees et compteur
            if (compteur<ListePointsCrees.Length-1){
            MainCamera.transform.rotation = Quaternion.Slerp(ListePointsCrees[compteur].transform.rotation,
                                                 ListePointsCrees[compteur+1].transform.rotation, TempsGlob[indice]);
            }
            
            Vector3 point_sub = new Vector3(XSubdivision[indice], 
                                        YSubdivision[indice], ZSubdivision[indice]);
            ListePoints.Add(point_sub);
            
            
            
            indice++;
            if (TempsGlob[indice]==1){
                compteur++;
            }
        }
    }
