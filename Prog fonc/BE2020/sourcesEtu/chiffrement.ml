...
struct
  ...
  type ('b, 'f) arbre_chiffrement = Noeud of (('b, 'f) branche list) | Lettre of 'f
  and ('b, 'f) branche = 'b * ('b, 'f) arbre_chiffrement
  ...
  (*  transforme un code (sous forme de liste des symboles de l'alphabet des codes) à l'aide d'un arbre **)
  ...
  let rec dechiffrer liste_dec arbre = 
    if liste_dec = [] then []
    else match liste_dec with
    |[] -> []
    |t::q -> match get_branche t arbre with
            |None -> raise CodeNonValide
            |Some(Lettre f) -> [f]@(defchiffrer q arbre)
            |Some(Noeud l) -> dechiffrer q (List.fold_right(fun t nouv_arbre -> if (nouv_arbre = None) then (get_branche t nouv_arbre)
                                                              else nouv_arbre) l None) 
