...
let tri_insertion ordre l = List.fold_right ( fun t qtrie -> insert ordre t qtrie) l []
...
let rec scinde l = 
  ...
  |[] -> ([], [])
  | [a]-> ([a], [])
  |a::b::q -> let  q1, q2  = scinde q in a::q1 , b::q2
...
let rec tri_fusion ordre l = 
  ...
  |[] -> []
  |[a] -> [a]
  | q -> let q1,q2 = scinde q in fusionne ordre (tri_fusion ordre q1) (tri_fusion ordre q2)
...
*)
let listStat = 
  let input = open_in "/home/tiffany/Documents/prog fonc/TP2/nat2016.txt" in 
...
*)
let listStatHomme = 
  let input = open_in "/home/tiffany/Documents/prog fonc/TP2/nathomme2016.txt" in
  let filebuf = Lexing.from_channel input in
  Parser.main Lexer.token filebuf
...
let listHommetrie = tri_insertion (fun x y -> x<y) listStatHomme
