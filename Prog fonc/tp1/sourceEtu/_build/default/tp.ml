...
let padovan2 n = 
   let rec pado padon_1 padon_2 padon_3 p =
      if p = n then padon_2 + padon_3
      else pado (padon_2 + padon_3) padon_1 padon_2 (p+1)
   in match n with
   |0 -> 0
   |1 -> 0
   |2 -> 1
...
let rec dragon (xa,ya) (xb,yb) n = 
   match n with 
   |0 -> dessine_segment (xa,ya) (xb,yb)
   |_ -> let (xc, yc) = ((xa+xb)/2 +(yb-ya)/2, (ya+yb)/2 + (xa-xb)/2) 
in dragon (xa, ya) (xc, yc) (n-1) and  dragon (xc, yc) (xb, yb) (n-1)
