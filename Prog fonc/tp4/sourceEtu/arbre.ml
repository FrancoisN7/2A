...
let%test _ = let nv_arbre = retrait_arbre ['b';'a';'t'] arbre_sujet in (appartient_arbre ['b';'a';'s'] nv_arbre && not( appartient_arbre['b';'a';'t'] nv_arbre) )
let%test _ = let nv_arbre = retrait_arbre ['l'; 'e'] arbre_sujet in (appartient_arbre ['l'; 'e'; 's'] nv_arbre && not( appartient_arbre['l'; 'e'] nv_arbre) )

(*****************************************************************************************)
(*   fonction qui renvoie la liste des éléments d'un arbre dans l'ordre lexicographique  *)
(*   signature  :liste_arbre : 'a arbre -> 'a list                                       *)
(*   paramètres : - un arbre n-aire                                                      *)
(*   résultat   : la liste de mots de l'arbre dans l'ordre lexicographique               *)
(*****************************************************************************************)

let rec liste_arbre (Noeud (b, lb)) = 
if b then [[]]@List.flatten(List.map(fun (car, Noeud(booleen, slb)) -> List.map (fun l -> car::l) (liste_arbre (Noeud(booleen, slb)))) lb) else List.flatten(List.map(fun (car, Noeud(booleen, slb)) -> List.map (fun l -> car::l) (liste_arbre (Noeud(booleen, slb)))) lb)


let%test _= liste_arbre (Noeud(true, [])) = [[]]
let%test _= liste_arbre (Noeud(false, [])) = []
let%test _ = liste_arbre arbre_sujet = [['b';'a';'s']; ['b';'a';'t']; ['d';'e']; ['l';'a']; ['l';'a';'i'];
['l';'a';'i';'d']; ['l';'a';'i';'t']; ['l';'a';'r';'d']; ['l';'e'];
['l';'e';'s']; ['l';'o';'n';'g']] 

(*****************************************************************************************)
(*   fonction qui élimine les branches inutiles d'un arbre                               *)
(*   signature  : elimination : 'a arbre -> 'a arbre                                     *)
(*   paramètres : - un arbre n-aire                                                      *)
(*   résultat   : un arbre n-aire sans les branches inutiles                             *)
(*****************************************************************************************)
let rec elimination (Noeud (b, lb)) = 
   match lb with 
   |[] -> if not b then (Noeud(false, maj [] (Noeud(false, [])) lb)) else Noeud(b, lb)
   |lb -> List.map(fun (car,(Noeud(booleen, slb))) -> elimination slb ) lb
...
let arbre_test = retrait_arbre ['l';'o';'n';'g'] arbre_sujet_e