...
(* evalue *)
(* evalue : float -> float liste -> float
Fonction qui evalue à partir de la formule de Horner
Parametre x : un réel
Paramètre l : un polynome codé avec une float list
Résultat : un réel résultat de la formule de Horner
*)
let evalue x l = List.fold_right(fun t1 res -> res *. x +. t1) l 0. 

let%test _ = evalue 0. p0 = 2.5
let%test _ = evalue 0. p3 = 4.5
let%test _ = evalue 1. p1 = 5.0 
let%test _ = evalue (-2.5) p1 = -5.5
let%test _ = evalue (-2.5) p2 = -24.25
...
let retract l = List.fold_right(fun t1 res -> if (t1=0. && res=[]) then res else t1::res) l []
...
let scal_mult x poly = retract (List.map(fun t -> x *. t) poly)
...
let rec plus poly1 poly2 = 
  match poly1, poly2 with
  | [], poly2 -> poly2
  | poly1, [] -> poly1
  | t1::q1, t2::q2 -> retract ((t1 +. t2)::(plus q1 q2))
...
let decoder code arbre = 
  recompose_chaine(
  let rec decoder_aux code_dec (Noeud(a,b,c)) = 
    match code_dec with 
    |[] -> []
    |t::q -> if t=1 then match a with
                | Lettre x -> x::(decoder_aux q a)
                | Vide -> raise CodeNonValide
                | _ -> if q=[] then raise CodeNonValide else decoder_aux q a
             else if t=2 then match b with
                | Lettre x -> x::(decoder_aux q b)
                | Vide -> raise CodeNonValide
                | _ -> if q=[] then raise CodeNonValide else decoder_aux q b
             else if t=3 then match b with
                | Lettre x -> x::(decoder_aux q c)
                | Vide -> raise CodeNonValide
                | _ -> if q=[] then raise CodeNonValide else decoder_aux q c
             else raise CodeNonValide
  in decoder_aux (decompose_int code) arbre)
