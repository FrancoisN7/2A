...
public class Irc {
	...
	public static void sendMessage(String Nom, String Objet, String Contenu){
        Irc.print(Nom + " -> " + Objet);
        Irc.print(Contenu);
        Irc.print("La liste des users est :");
        for (String u : users) {
            Irc.print(u);
         }
    }
...
class writeListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			StreamMessage sm = Irc.session.createStreamMessage();
            sm.writeString(Irc.data.getText()) ;
            Irc.producer.send(sm);
...
class connectListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			// Une instance de la Connection Factory
            ...
            MessageListener listener = new MessageListener() {
                public void onMessage(Message msg) {
            		try {
        				...
        				String Nom = sm.readString();
        				if (!Irc.users.contains(Nom)){
        				    Irc.users.add(Nom);
        				}
        				String Objet = sm.readString();
        				String Contenu = sm.readString();
        				Irc.sendMessage(Nom, Objet, Contenu);
